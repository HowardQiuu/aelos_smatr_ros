...
# 添加颜色锁定相关变量
current_color = None  # 当前正在处理的颜色
color_locked = False  # 颜色是否已锁定
search_attempts = 0  # 搜索尝试次数
max_search_attempts = 5  # 最大搜索尝试次数

# 不同色块的hsv范围
color_range = {
    'green': [(43, 233, 97), (50, 255, 159)],
    'orange': [(8, 231, 139), (12, 255, 235)]
}

# 颜色优先级列表（可根据需要调整优先级）
color_priority = ['green', 'orange']  # 绿色优先级高于橙色
...
def box_go_selfdef(n): # 抱着箱子前进一步
    for i in range(0, n):
        base_action.action('BoxForward')
        time.sleep(1)
...
def find_box(img, color_name):
    ...
    if Chest_img is None:
        ...
        if len(contours) != 0:
            ...
            # 只接受面积大于一定阈值的方块，避免误检
            if area[max_index] > 50:  # 可根据实际情况调整阈值
                ...
                print(f'找到{color_name}方块', 'x=', chest_circle_x, 'y=', chest_circle_y)
...
# 搬箱子 - 修改版，添加颜色参数
def goto_box(color_name):
    global level, ID, current_color, color_locked
    
    if chest_circle_x is None:
        print(f'等待获取{color_name}方块坐标中...')
        time.sleep(0.3)
        return False
    else:
        # 锁定当前颜色
        if not color_locked:
            ...
            color_locked = True
            print(f'锁定处理{color_name}方块')
        
        if chest_circle_x < 275:
            print(f"正在左侧移对准{color_name}方块 ", chest_circle_x)
            ...
            print(f"正在右侧移对准{color_name}方块 ", chest_circle_x)
            ...
            if chest_circle_y < 300:
                print(f"前进接近{color_name}方块", chest_circle_y)
                base_action.action("FastForward1s")
                time.sleep(0.5)
            elif chest_circle_y >= 340:
                print(f"后退调整{color_name}方块距离", chest_circle_y)
                base_action.action("Back1Run")
                time.sleep(0.5)
            else:
                print(f"开始抱{color_name}箱子")
                base_action.action("Forwalk01")
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if ID == 0:  # 搬箱子
            if level == "start_box":
                # 重置搜索计数
                if not color_locked and search_attempts > max_search_attempts:
                    search_attempts = 0
                    chest_circle_x = None
                    chest_circle_y = None
                
                # 如果已经锁定颜色，继续处理该颜色
                if color_locked and current_color:
                    found = find_box(Chest_img, current_color)
                    if found:
                        search_attempts = 0  # 重置搜索计数
                        goto_box(current_color)
                    else:
                        search_attempts += 1
                        if search_attempts > max_search_attempts:
                            print(f"无法找到{current_color}方块，解除颜色锁定")
                            color_locked = False
                            current_color = None
                            chest_circle_x = None
                            chest_circle_y = None
                    ...
                    for color in color_priority:
                        if find_box(Chest_img, color):
                            color_found = True
                            print(f"发现{color}方块，开始处理")
                            goto_box(color)
                            break
                    
                    if not color_found:
                        search_attempts += 1
                        print(f"未发现任何方块，继续搜索... ({search_attempts}/{max_search_attempts})")
                        # 可以在这里添加搜索动作，如转身寻找
                        if search_attempts > max_search_attempts / 2:
                            # 搜索超过一半次数后，可以执行转身等动作寻找方块
                            R_turn1(1)  # 小幅度右转寻找
            ...
            marker = Tag.get_nearest_marker() # 获取最近的AR标签

            if len(marker) == 0:
                print("无tag")
                if (ID == 1 and level == "end_box"): # 刚抱上箱子，在找标签
                    print("右转")
                    BoxR_turn2(1)
                elif (ID == 1 and level == "start_moving") or ID == 2 or ID == 3 or ID == 4 or (ID == 5 and step == 1): # 在前进阶段，找标签
                    print("抱着箱子后退")
                    Box_Back(1)
                elif (ID == 5 and step == 2 and level == "start_moving"): # 放下箱子，需要反向找标签
                    print("右转")
                    R_turn2(1)
                elif (ID == 6) or (ID == 7) or (ID == 5 and step == 2 and level == "reverse_moving"): # 返回阶段找标签
                    print("后退")
                    Back2(1)
                ...
                if step == 1:
                    ...
                    elif marker[0] == 4: # 识别到四号标签，需要对其
                        if ID == 4: # 识别到四号标签，需要对其
                    ...
                    elif marker[0] == 5: # 识别到五号标签，需要对其
                        if ID == 5: # 识别到五号标签，需要对其
                            result = turn_to_tag(robot_tag_x, robot_tag_y, tag_yaw, 0.12, 0.0, 0)
                            if result == True: # 对其到了五号标签，前进放下海绵块，进入step=2回去阶段
                    ...
                    # 反方向
                    if marker[0] == 5: # 放下后还需要精准识别对其上五号，以左转对其六号
