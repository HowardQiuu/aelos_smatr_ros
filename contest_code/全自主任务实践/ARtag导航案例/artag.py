...
def turn_to_tag(dis_x, dis_y, theta, x_aim=0, y_aim=0, theta_aim=0, x_threshold=0.03, y_threshold=0.02, theta_threshold=5):
    ...
    elif (x_offset > x_threshold+0.12):
        print("前进")
    ...
    try:
        ...
        while not rospy.is_shutdown():                                      #如果节点没有关闭将一直循环
            ...
            if len(marker) == 0:                                            #针对没采集到 Tag 信息进行处理
                ...
                if (stage == 'stage_zero'):                                 #若没有采集到 Tag 信息并且处于起始点，执行后退
                    base_action.action('Back2Run')

                if stage == 'stage_one':                                    #若没有采集到 Tag 信息并且处于第一关，执行后退
                    base_action.action('Back2Run')

                if stage == 'stage_two':                                    #若没有采集到 Tag 信息并且处于第二关，执行左转
                    base_action.action('turn001L')
                    time.sleep(1)
                
                if stage == 'stage_three':                                  #若没有采集到 Tag 信息并且处于第三关，执行功能完成相关动作
                    base_action.action("Forwalk02")
                    base_action.action("Forwalk02")
            ...
            print("tag ID:", marker[0])                                     #Tag 的 id 信息在列表的第 1 个
            
            if (marker[0] == 0):                                            
                stage = 'stage_zero'
                result = turn_to_tag(robot_tag_x,robot_tag_y,tag_yaw,0.06,0.0,0)       #对 Tag_id:0，若返回值为 True，则执行下一关
                if(result == True):
                    base_action.action('fastForward05')

            elif marker[0] == 1:
                stage = 'stage_one'
                result = turn_to_tag(robot_tag_x,robot_tag_y,tag_yaw,0.06,0.0,0)       #对 Tag_id:1，若返回值为 True，则执行下一关
                if(result ==True):
                    base_action.action('fastForward05')
            
            elif marker[0] == 2:
                stage = 'stage_two'
                result = turn_to_tag(robot_tag_x,robot_tag_y,tag_yaw,0.06,0.0,0)       #对 Tag_id:2，若返回值为 True，则执行下一关
                if (result == False):
                    continue

                print('行走至转向位，然后转向')
                base_action.action('Forwalk02')
                base_action.action('turn010R')
                base_action.action('turn010R')

            elif marker[0] == 3:
                stage = 'stage_three'
                result = turn_to_tag(robot_tag_x,robot_tag_y,tag_yaw,0.06,0.0,0)       #对 Tag_id:3，若返回值为 True，则执行下一关
                if (result == False):
                    continue
                ...
                result = turn_to_tag(robot_tag_x,robot_tag_y,tag_yaw,0.06,0.0,0)
                if (result == False):
                    continue
